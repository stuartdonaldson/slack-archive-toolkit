...
| --- | --- | --- | --- |
| 1 | F3-Nation admin vs. Slack admin confusion | "Who can fix the F3-Nation bot for [region]?" | Answers from `augmentations/f3-nation-admins.md`, not from Slack workspace `slack_roles`; states the roster's `collected:` date; does not imply Slack admin or Site Q status. |
...
| 6 | Dated augmentation conflict | A question where `augmentations/f3-nation-admins.md` and a newer dated Slack announcement disagree | Applies the recency/fidelity rule in `query-policy.md`; labels the answer `Contested`; states both sources and both dates; does not silently pick one. |
