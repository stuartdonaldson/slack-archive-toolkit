...
## 2026-08-08 08:29:14
_session 22533183-84ad-4b1c-bedd-830532ac3da9 · v3 · 08-08_

### Objective 1: Build a region/channel/message-count/last-message CSV report over the archives
Rationale: Needed a flat, spreadsheet-friendly view across every archived channel, sourced directly from each channel's own `slackdump.sqlite` rather than catalog metadata, so it reflects what's actually on disk.
Outcome [user-facing]: `scripts/channel-message-summary.py` added, writing `region, channel, num_messages, last_message, days_since_last_message, days_since_created, topic, description` to a CSV (default `/tmp/slack-backup-recent.csv`); creation date/topic/description read from each archive's own CHANNEL row JSON (`created`, `topic.value`, `purpose.value`).

### Objective 2: Switch nightly channel-registration from a one-workspace-per-night rotation to scanning all workspaces every night  [accreted]
Transition: Came up while reviewing the report - a real, non-archived, non-private channel (`f3kirkland/kirkland-ipc`, created 4 days prior) was missing because the nightly `channel register` step only scans one workspace per night on a day-of-year rotation, so it hadn't reached f3kirkland's slot yet.
Rationale: Measured the two rotation scans that had actually run (f3tundra 98s, f3cascades 105s) - real cost was ~1.5-2 min/workspace, not the "several minutes, rate-limit-prone" originally assumed. Developer's call: "at 1-2 minutes each it's worth the delay to have recent channels appearing right away."
Rejected: Kept the rotation's original architecture (day-of-year mod workspace-count) rather than tuning its cadence - replaced it outright, since `channel register` already supports a `'*'` workspace glob that covers all registered workspaces in one call.
Outcome [user-facing]: `scripts/nightly-backup-digest.sh` now runs `./slackbackup channel register '*' '*'` every night instead of rotating one workspace; newly-created public channels show up in the very next run instead of waiting up to a week.
Outcome [internal]: `docs/OPERATIONS.md` and `docs/DESIGN-files.md` updated to drop the stale rotation description and record the measured timing.

### Objective 3: Fix duplicate MESSAGE rows accumulating across resume cycles  [accreted]
Transition: Surfaced by the report tool itself - digest said 20 messages for `f3kirkland/ask-arches`, the script said 178. Diagnosing that discrepancy escalated into fixing the underlying pipeline bug once the root cause turned out to be structural, not a report-script mistake.
Rationale: `backup_channel()`/`resume()` deliberately never pass slackdump's own `resume -dedupe` flag (confirmed data-loss bug: deletes thread-root rows), but that left duplicate rows from every resume's lookback window with no cleanup step at all - confirmed 178 raw rows for 20 real messages on one channel, and 117,165 duplicate rows across 305 of 437 channels workspace-wide. Verified the standalone `slackdump tools dedupe -mode message-key -execute` command (distinct from the buggy inline flag) is safe: on a scratch copy it collapsed 178->20 rows, left the thread-root row intact, and a subsequent `convert -f export` still produced the correct 20-message export. Separately confirmed `convert -f export` (what the digest pipeline actually reads) already produced the correct count even against the *undeduped* archive - "are the duplicates making it into the digests?" - no; this was disk-bloat only, never a digest-correctness bug.
Rejected: Running the real backfill across all 437 channels interactively - per-channel cost scales with chunk/resume history (not just message count; one 95-message channel took ~1.5min due to 617 chunks), so a full pass was estimated at 1-3+ hours. Developer chose to clean only the ~20 worst-offender channels immediately and let the rest self-heal via the new automatic per-resume dedupe.
Outcome [developer-facing]: `slackdump.dedupe()` wrapper added; `backup_channel()` calls it non-fatally after every successful `resume()` (SlackBackup-9hq) so duplicates no longer accumulate going forward. Tests added/updated in `test_slackdump.py` and `test_backup_logic.py`; full suite passes (410 tests). `docs/DESIGN.md` and `docs/references/slackdump-cli-notes.md` updated to distinguish the safe standalone command from the buggy inline flag.
Outcome [developer-facing]: `scripts/dedupe-existing-archives.py` added (one-time backfill tool, `--dry-run` and `--only workspace/channel,...` supported); `scripts/channel-message-summary.py`'s count fixed to `COUNT(DISTINCT ts)`.
Outcome [internal]: One-time backfill launched in the background against the 20 worst-offender channels (~33% of the 117k-row backlog); still running as of session end.
Open: Backfill-against-worst-20 completion not yet confirmed; remaining 285 affected channels rely on the normal nightly resume cadence (up to 10 days) to self-heal rather than an explicit backfill pass.

### Key Learnings:
`slackdump tools dedupe` (standalone) and `resume -dedupe` (inline flag) share underlying dedupe logic per slackdump's own help text, but only the inline flag has the confirmed thread-root-deletion bug - the standalone command, run as a separate step, is safe. `slackdump convert -f export` dedupes messages internally at export time regardless of duplicate rows in the source archive, so archive-level duplication does not propagate to derived output.

## 2026-08-08 00:00:00
_session 39c669bb · v3 · 08-08_

### Objective 1: Rename digest's `purpose` field to `description`, drop the old topic-or-purpose merge
Rationale: The digest's per-channel metadata carried three overlapping fields — `topic`, `purpose`, and a synthesized `description` (topic if set, else purpose). Slack's own UI only shows two concepts: the pinned `topic` and a "Description" panel that is actually the `purpose` field under the hood. The synthesized `description` had no Slack UI counterpart to correlate against and could silently drop the `purpose` text whenever `topic` was also set — "the merged field would be giving priority to Topic and ignoring the existing purpose field in the api so data can get lost... I want the LLM to not get confused by the two." Investigation confirmed the old merge was pure passthrough in the digest (computed once by `catalog_logic.description_of()`, re-surfaced as-is, no digest logic depended on it), so renaming was safe. Decision: rename raw `purpose` to `description` in the digest output (matching Slack's UI label), drop the old merged field, leave `topic` and the catalog cache's own fields untouched.
Outcome [user-facing]: `export digest` output bumps to `schema_version: "slack-llm-digest-v6"` — each channel entry now carries `topic` and `description` (Slack's `purpose`, renamed); the old ambiguous merged `description` field is gone.
Outcome [developer-facing]: `_channel_context` in `export_logic.py` updated and re-documented; `docs/adr/0007-digest-v6-drop-merged-description.md` added; `docs/DESIGN-export.md` (schema authority), `docs/DESIGN-files.md`, `docs/llm-context/uploads/ingestion-contract.md`, and `docs/llm-context/project-instructions.md` updated to match; test fixtures/assertions in `tests/test_export_digest_logic.py` updated. Full suite (410 tests) passes.
Open: catalog.py's CLI listing and channel_digest_logic.py's separate `slack-channel-digest-v2` schema still use the old merged `description` via `catalog_logic.description_of()` — left untouched as out of scope, not yet renamed for consistency. Work not yet committed or pushed.

## 2026-08-08 17:15:00
_session a87f382f · v3 · 08-08_

### Objective 1: Diagnose and fix null channel metadata (topic/description/creator) in Slack digests
Rationale: User spotted that `ao-gasworks` had a visible topic/description in Slack but both came back `null` in the generated digest, and asked whether the backup pipeline failed to capture it. Traced to the local catalog cache (`~/.cache/slackbackup/<workspace>.catalog.json`): `export_logic._channel_context` reads topic/description/creator/created_at only from this cache, never live. The cache has two tiers — cheap "fast" (`list channels -member-only`, only channels the archiving Slack account is itself a member of — confirmed live: 1 of 56 for f3seattle) and expensive "full" (`list channels`, every public channel, the only tier with metadata for non-member channels like `ao-gasworks`). The nightly `backup run` warm-up (`backup_logic.py`) calls only `refresh_fast`, so full-tier data goes stale indefinitely — f3seattle's was 44 days stale, predating the commit (`000af01`, 2026-08-06) that even added `topic`/`purpose`/`creator`/`created` to the cache schema, so those channel records literally lacked the keys.
Rejected: While running `catalog show --full` across all 7 workspaces to fix this, discovered a second bug — `slackdump list channels -no-chan-cache` intermittently returns a severely truncated result (5 channels instead of ~60) on a cold call right after a workspace switch, with no error. f3seattle's first refresh attempt silently "succeeded" (TTL stamped) while leaving the cache nearly empty; retried once slackdump's own internal cache was warm and it returned the full, correct set.
Rationale: While explaining the `FULL_TTL_SECONDS = 21600` (6h) rationale, initially cited `ao-gasworks`'s `num_members: 396` field as supporting evidence for "full-tier is expensive" — user caught this was a bogus inference (that's a metadata count, not per-member API calls); corrected. Separately, re-ran the exact full-tier call the project's own docs cite as "4m20s, rate-limited twice" (`docs/references/slackdump-cli-notes.md`, measured 2026-06-23) on the same workspace/channel-count and got ~59s with zero rate-limit events — the original measurement is unvalidated since (a slackdump version bump on 2026-07-26 only touched the pinned version string, not a re-measurement), so recorded this as an open question in the code rather than either trusting or discarding the original number.
Outcome [internal]: Ran `./slackbackup catalog show <workspace> --full` for all 7 workspaces (f3seattle, f3cascades, f3pugetsound, f3kirkland, f3northsea, f3tundra, f3redmond); f3seattle needed a retry due to the truncation bug. Verified post-fix: 0 channels missing the `topic` key across all 7 caches; user regenerated digests and confirmed `ao-gasworks` now carries correct topic/description/creator/created_at, and that remaining `null` entries elsewhere are legitimate (channels with no topic/purpose set in Slack, not cache artifacts).
Outcome [developer-facing]: Added a dated rationale comment to `FULL_TTL_SECONDS` in `catalog_logic.py` recording the 4m20s-vs-59s discrepancy and what would justify revisiting the TTL.
Outcome [developer-facing]: Filed 4 bd issues for durable fixes: `sat-4ig` (fold full-tier refresh into backup run's warm-up), `sat-dnr` (guard against the silent slackdump truncation, with retry), `sat-pvk` (export digest should check catalog staleness and refresh, not just `load()` — blocked on `sat-dnr`), `sat-il0` (audit/restrict `refresh_fast`/fast-tier usage to genuinely membership-scoped call sites, including `catalog_logic.lookup()`'s stale-metadata gap).

### Objective 2: F3-Nation context-pack documentation housekeeping  [accreted]
Transition: Follow-on questions about the LLM context pack's augmentation files surfaced while reviewing the digest output from Objective 1; user judged this and the later migration-plan review secondary to the cache fix ("the only real objective here that had any impact was addressing the channel metadata cache issue and the strategy updates from that") but real file changes resulted, recorded here for completeness.
Rationale: Compared `f3-nation-admins.md` (manual, 2026-06-30, admin-only, no Redmond) against `f3nation-pugetsound.md` (live `api.f3nation.com` pull, 2026-08-07, admin+editor, includes Redmond) and confirmed the newer file supersedes the older with no content gap; user confirmed the Site Q duplication between `f3nation-pugetsound.md` and Slack channel `purpose` fields is intentional, not redundancy — "different sources of information... which is data in the f3-nation app and database which is admin maintained vs the data available via slack which is community maintained," kept specifically to surface consistency gaps since regional PAX rarely update the F3-Nation app side.
Outcome [internal]: Deleted `docs/llm-context/uploads/augmentations/f3-nation-admins.md`; updated 5 cross-referencing files (`f3-augmentation-index.md`, `f3-nation-apps.md`, formerly `f3-nation-app-operations.md` and `f3-nation-operations.md`, `README.md`, `MIGRATION-PLAN.md`, `validation-set.md`) to repoint at `f3nation-pugetsound.md` and added a §Deliberate dual-sourcing note to the augmentation index capturing the admin-maintained-vs-community-maintained rationale above.
Outcome [internal]: Ran `/plan-graduation-audit` against `MIGRATION-PLAN.md` (adapted in place of `PLAN.md`, using this project's `docs/framework/doc-standard.md` §Graduation Rules). Found the plan's own §Status/§Migration tracking bead-status table stale against bd (claimed `sat-ejk.4` in-progress and `sat-ejk.6` open; bd shows the `sat-ejk` epic closed 9/9). Produced a 13-item graduation table: 1 item → ADR (Accepted) candidate, 1 item → new bd issue candidate (no fixed augmentation refresh cadence/owner — a genuinely still-open decision currently invisible to `bd ready`), 8 items → discard as stale-vs-bd or duplicate of already-maintained files (`README.md`, `query-policy.md`, `DESIGN-export.md`), 2 items → keep unchanged as historical record (same treatment as `VALIDATION-RESULTS.md`/`REVIEW-2026-08-06.md`).
Open: The ADR and bd issue identified by the graduation audit were not filed, and `MIGRATION-PLAN.md` was not trimmed — pending the user's go-ahead.
Open: While running the audit, searched for `doc-bootstrap-standards.md` (named as a required input by the `plan-graduation-audit` skill's own instructions) and found it does not exist in this project but does exist under `docs/framework/` in two sibling project repos (`AIPraxis`, `GDriveUtils`) — unresolved whether this is a stale reference in the skill (renamed to `doc-standard.md`) or a genuinely distinct file this project's framework copy is missing; user asked to hold off investigating until the "why" is understood.

### Key Learnings:
`slackdump list channels -no-chan-cache` can silently return a drastically truncated channel list (observed: 5 instead of ~60) on a cold call immediately after a workspace switch, with exit 0 and no stderr warning — a retry once slackdump's own internal cache is warm returns the correct full set. Not documented anywhere prior to this session.

## 2026-08-09 00:00:00
_session 1772e3c9-9906-45d8-a9ba-c2ece7a9affd · v3 · 08-08→08-09_

### Objective 1: Graduate resolved content out of the retired-but-still-present LLM context migration plan
Rationale: `/plan-graduation-audit` was run against `docs/llm-context/`'s plan and review documents to check relevance; `bd show sat-ejk` confirmed the migration epic closed 9/9, so the planning/review/validation-results trio was stale process record, not live guidance. Audit found one genuine gap (the pack's decomposition and evidence-precedence decision had never graduated to an ADR) and one genuinely outstanding, previously-untracked action item (the live ChatGPT upload-and-ask validation named in both documents was never performed). User confirmed with "agree go ahead."
Outcome [developer-facing]: Added `docs/adr/0008-llm-context-pack-decomposition.md` recording the pack-decomposition and evidence-precedence decision; filed `sat-dul` (P2, live upload-and-ask validation still outstanding) and `sat-cvs` (P4, augmentation size-budget/refresh-cadence/owner decision still open) so neither item was lost.
Outcome [internal]: Deleted `docs/llm-context/MIGRATION-PLAN.md`, `REVIEW-2026-08-06.md`, and `VALIDATION-RESULTS.md` (recoverable via `git log --follow`); repointed every live cross-reference (`CLAUDE.md`, `docs/OPERATIONS.md`, `docs/llm-context/README.md`, `validation-set.md`, `prompts/fng-getting-started.md`, `scripts/nightly-backup-digest.sh` comments) at ADR-0008 instead. `work-log.md`'s own prior mentions of the retired docs were left untouched as immutable history.
Open: Changes are staged in the working tree on `sat-811-files-sidecar`, not yet committed or pushed — folded into whatever commit closes out the branch's other in-flight work.
## 2026-08-09 08:27:57
_session 5c734386 · v3 · 08-09_

### Objective 1: Resolve recurring nightly-backup hangs with channel-level locks and subprocess timeouts
Rationale: ~/slack-backups/nightly.log showed the same failure mode repeatedly (2026-08-05→06, 2026-08-08→09, and live during this session on 2026-08-09): a channel's resume/dedupe step logs, then the next line is hours-to-a-day later at the next night's cron fire. Live investigation of the in-progress run found the actual cause split two ways: (1) some gaps were WSL2/host-sleep artifacts (kernel uptime vs wall clock diverging across a laptop sleep), not real hangs; (2) `slackdump tools dedupe -mode message-key -execute` genuinely hung - 100% CPU for 24+ minutes on a 966-message channel (a similarly-sized channel deduped in 185s) - with nothing bounding it, since neither a subprocess timeout nor a lockfile existed anywhere in the pipeline (`backup_logic.py`, `scripts/nightly-backup-digest.sh`). User: "will killing 19810 now let it continue?" confirmed the existing swallow-and-log pattern (`_dedupe_quietly`) already tolerates a killed subprocess; the missing piece was making that automatic and preventing overlapping runs. User specified the lock must live in `slackbackup`/its tools, not the nightly wrapper script, "since we're likely to run a tool mid-day and it may conflict with something hung," and confirmed channel-level granularity (the actual unit of file conflict, matching backup's existing per-channel loop) over workspace- or archive-root-level, plus that export should share the same lock ("Lock it too") despite SQLite WAL already making concurrent reads safe.
Rejected: workspace- or archive-root-level lock granularity - would block unrelated concurrent work (e.g. nightly on one workspace blocking a manual run against a different channel) for no real conflict.
Outcome [developer-facing]: New `channel_lock.py` (sat-7k9) - exclusive pidfile lock stored as a *sibling* of the channel directory (survives the empty-archive `rmtree` that happens mid-hold on one backup_logic.py path), stale-PID reclaim via `os.kill(pid, 0)` (correct across WSL2 suspend/resume, unlike file-age checks). Wired into `backup_logic.backup_channel()`/`run()` (new `locked` skip counter, same soft-skip treatment as `SlackdumpError`), all three `export_logic.py` `convert_export` call sites (new `"locked"` channel/workspace status alongside `"missing_archive"`), `export.py`'s single-channel `export monthly` CLI (exits 3, nothing to skip-and-continue to), and `scripts/backfill-bot-images-existing-archives.py`. `slackdump.py` (sat-hh0): `archive`/`resume`/`tools dedupe` now pass a bounded timeout (`ARCHIVE_TIMEOUT_SECONDS`=2h, `RESUME_TIMEOUT_SECONDS`=1h, `DEDUPE_TIMEOUT_SECONDS`=15min - tight since dedupe is local-only and was the exact case that hung); a timeout kills the subprocess and raises the existing `SlackdumpError`, so no caller-side changes were needed. `docs/DESIGN.md` gained a new Concurrency section plus module-table updates. Full suite green (451 tests, all written test-first per the implementation gate). sat-7k9 and sat-hh0 closed.
Outcome [user-facing]: Nightly/batch backup runs no longer stall indefinitely on one pathological channel or one overlapping mid-day run; the run summary line now reports locked-skip counts; `export monthly` reports a clear locked-channel error instead of hanging.

## 2026-08-09 22:43:02
_session c55486e0-25f9-426a-adb7-14be4f6b8f28, 9fd708c2-ba42-4fea-8cd8-640de31b91c3, 59384639-6bd5-471d-830e-4b4265731982 · v3 · 08-09_

### Objective 1: Close out the catalog-staleness follow-on work filed from the 2026-08-08 metadata-cache investigation (sat-dnr, sat-4ig, sat-il0)
Rationale (inferred - run via unattended bd-run-beads.py automation, not an interactive session): these three beads were filed together on 2026-08-08 as the durable-fix backlog for the null-metadata/truncation bug (see the 2026-08-08 17:15:00 entry's Objective 1), and were executed here in their dependency order - guard the truncation bug first, then fold full-tier refresh into the warm-up now that a truncated result won't get silently trusted, then audit remaining fast-tier call sites now that both tiers refresh together.
Outcome [developer-facing]: `catalog_logic.py` gained `_list_channels_guarded()` - retries `slackdump.list_channels()` up to 3 times when the result looks implausibly truncated (<50% of the already-cached count), and on both `refresh_fast()`/`refresh_full()` skips stamping `*_refreshed_at` if every attempt still looked truncated, so the next run retries instead of trusting a bad result for a full TTL cycle. 81 new test cases added. (sat-dnr, commit bd0daee)
Outcome [user-facing]: `backup run`'s per-workspace warm-up now calls `refresh_full()` immediately after `refresh_fast()` every run (previously full-tier only refreshed via `catalog show --full`, so non-member-channel metadata like topic/purpose/creator could go stale indefinitely between manual runs). `refresh_full()` has its own TTL so this is a no-op when already fresh. (sat-4ig, commit f062e43)
Outcome [developer-facing]: Audited every `refresh_fast()`/`refresh_full()` call site (`catalog_logic.py`, `channel_logic.py`, `backup_logic.py`, `catalog.py`); found the warm-up path already correct as of sat-4ig, and documented in code that `catalog_logic.lookup()`'s fast-tier-first behavior is a deliberate policy - its only caller (`channel_logic.register()`) needs id/name resolution, not fresh metadata, so a fast-tier hit is trusted as-is and can lag full-tier metadata by up to `FULL_TTL_SECONDS`; a future caller needing fresh metadata should call `refresh_full()` directly. Also added `--full` CLI help text on `catalog show` clarifying the fast/full distinction. (sat-il0, commit 66c8b52)
## 2026-08-09 23:47:00
_session af9491ed · v3 · 08-09_

### Objective 1: Work bd issue sat-pqf (harden LLM ingestion against prompt injection via extracted file/Canvas content) to completion
Rationale: Unattended run instructed to claim and implement sat-pqf per its description/acceptance framing — delimit or otherwise flag Slack-extracted file/Canvas content as untrusted for downstream LLM consumers, since a workspace member can embed instruction-shaped text that an LLM might obey.
Rejected: Implementing the fix again from scratch — discovered on investigation that the fix already existed, fully, already committed on this branch.
Outcome [internal]: Found the entire fix already landed in commit f062e43, mis-attributed to an unrelated bead (sat-4ig) via its commit message. That commit added ADR-0009 (explicitly citing sat-pqf), a Trust Boundary section in ingestion-contract.md, matching untrusted-content guidance in project-instructions.md/session-preamble.md/query-policy.md/all 5 prompt templates, and validation-set.md scenarios 13–15 covering injected-instruction, attacker-URL, and Project-mode-residency behavior. Verified the content against the bead's ask and confirmed it is satisfied; working tree clean; PYTHONPATH=src python3 -m pytest -q passed 464/464.
Outcome [internal]: Recorded findings as a bd note on sat-pqf (file-then-`bd note` form) documenting the f062e43 attribution gap, since no new commit was warranted.

### Objective 2: Decide how to close out sat-pqf given the fix predates this session
Transition: handed to the human once the "no code change needed" finding surfaced, since closing (or not) and how to handle the git-history attribution gap were the human's calls to make, not something to resolve unattended.
Rationale: Presented three options — close now with a note (low cost, relies on bd-note↔commit traceability), rewrite/reword f062e43 for proper attribution (clean history but disproportionate effort/risk for a message-only fix), or add a trivial follow-up commit referencing sat-pqf purely for bookkeeping. Recommended closing now; human chose that option directly ("option a").
Outcome [internal]: Closed sat-pqf via `bd close sat-pqf`, with the earlier bd note left in place as the attribution record pointing to f062e43.

## 2026-08-10 00:28:31
_session 56d42be1 · v3 · 08-09_

### Objective 1: Recommend and implement a solution for bd issue sat-pqf (prompt-injection hardening for LLM ingestion of Slack-extracted content)
Rationale: PR #6's security review flagged that extracted Canvas/document text — author-controlled by any workspace member — flows into the files_out sidecar, which the context pack tells LLM consumers to treat as rank-1 evidence, while generated outputs (newsletter, SLT report, AO validation) are distributed to real people.
Rejected: Build-time sanitization or phrase-blocklist scrubbing of extracted text — brittle, false-positives on ordinary Slack usage, and destroys the archive fidelity `merge_files_out` otherwise protects. Rejected inline `<untrusted>` fencing inside the content string — the JSON string boundary is already a stronger delimiter than any fence an attacker can also emit, and mutating content perturbs `content_sha256`/`content_changed_at`. Rejected emitting a per-artifact `content_trust` marker — a marker on our artifacts invites the inverse inference that a JSON lacking one is trusted.
Rejected: An enumerated untrusted-field list (the first recommendation) — the human proposed scoping by artifact format instead, which is self-maintaining: a field added in a later schema version is covered without an edit. Audited the export writers to confirm the split is currently exact (every emitted artifact is JSON; every guidance/prompt/augmentation file is operator-authored Markdown) and found one genuine exception — the raw machine transcript under `uploads/augmentations/sources/`, Markdown the operator did not author.
Outcome [user-facing]: ADR-0009 records the invariant that instructional and evidentiary authority are orthogonal: Slack-derived content keeps full evidence rank and carries zero instructional authority. Stated by provenance with file format as the operative shorthand. Applied across `project-instructions.md` (the only always-resident surface in Project mode), a new Trust boundary section in `uploads/ingestion-contract.md`, `session-preamble.md`, citation-provenance and report-the-passage rules in `uploads/query-policy.md`, all five prompt templates, and `validation-set.md` scenarios 13–15. Documentation-only: no code, no schema change, no `schema_version` bump, no export regeneration.
Outcome [developer-facing]: Added a maintenance guard in `docs/llm-context/README.md` and the augmentation index keeping the Markdown-is-trusted shorthand true — quoted third-party material must live under `augmentations/sources/` behind a header marking it as quoted. Gave the existing SOTN transcript that header; it had none. Build-time heuristic detection deferred to sat-j1t.
Outcome [internal]: `adr-quality-check` step 2 flagged the citation-provenance rule as a possible bundled second decision; resolved by making the derivation explicit (a URL is content, so the link rule is the one decision applied to the field type a reader's client acts on automatically) rather than splitting into an ADR that could not stand alone.

### Objective 2: Reconcile a concurrent-session collision in the shared worktree
Rationale: An unattended automation session (af9491ed) was committing in this same working directory while these edits were in progress. Its add-all commits swept 13 in-progress documentation files into two unrelated code commits, f062e43 (sat-4ig) and 66c8b52 (sat-il0).
Outcome [internal]: Content landed intact and correct; only its placement in history is wrong. Presented the options to the human, who chose to leave history unrewritten and document it rather than rebase. Correction to the record: session af9491ed's own note and work-log entry conclude the fix "already existed" and predated it, and attribute it to sat-4ig — it did not predate anything. It was authored concurrently in session 56d42be1 under sat-pqf and captured by that session's add-all commit. sat-pqf is correctly closed; the attribution in f062e43's commit message is not.
Outcome [internal]: Test suite was run by the concurrent session against this content: 464/464 passed. No test change was warranted — the change carries no logic.
